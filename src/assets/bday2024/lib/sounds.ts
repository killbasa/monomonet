...
export const setupSmooches = async (): Promise<void> => {
	const button = document.getElementById('cibo-audio-button');

	const urls: string[] = [];
	for (let i = 1; i <= 25; i++) {
		urls.push(`/bday2024/sounds/smooch${i}.mp3`);
	}
